...
void reset_level(){
	if (level == 1){
		...
		// TREASURE
		sprite_init(treasure, screen_width() * 0.5, screen_height() * 0.5, TREASURE_WIDTH, TREASURE_HEIGHT, treasure_image);
		// Create bat sprite
...
void start_level(void){
	if (level == 2) {
		...
		// TREASURE
		treasure = sprite_create(screen_width() * 0.5, screen_height() * 0.5, TREASURE_WIDTH, TREASURE_HEIGHT, treasure_image);
		...
		// KEY
		sprite_init(barrier_key, screen_width() * 0.93 - 1, screen_height() * 0.1 , KEY_WIDTH, KEY_HEIGHT, key_image);
		key_taken = false;
		// TREASURE
